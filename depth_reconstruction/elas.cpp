...
void StereoEfficientLargeScale::process(cv::Mat& leftim, cv::Mat& rightim, cv::Mat& leftdisp, cv::Mat& rightdisp, int bd)
{
	Mat l,r;
...
{
	Mat temp;
	StereoEfficientLargeScale::process(leftim,rightim,leftdisp,temp,bd);
}
/*
void StereoEfficientLargeScale::check(Mat& leftim, Mat& rightim, Mat& disp, StereoEval& eval)
{
	...
	while(key!='q')
	{
		...
		process(lim,rim,disp16,dispr16,16);
